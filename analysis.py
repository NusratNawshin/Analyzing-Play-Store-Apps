...
originals=numerical_columns[numerical_columns.columns[1:]]
...
# I am doing 2 data Transformation in this project. 
...
unique_count = play_store_app_df_removed_outlier.groupby(['Category'], as_index=False).agg(Total_Count=('Category', pd.Series.count))
print(unique_count)
rcParams['figure.figsize'] = 15,8

sns.barplot(x='Category',y='Total_Count',data=unique_count)

# plt.figure(figsize=(15,8))
plt.xticks(rotation=90)
plt.title("Count of App Categories")
plt.tight_layout()
plt.grid(axis='y')
plt.show()
...
plt.xticks(rotation=90)
plt.title("Count of App Categories by Ad Supported")
plt.tight_layout()
plt.legend()
plt.grid(axis='y')
plt.show()
...
unique_count_Free = play_store_app_df_removed_outlier.groupby(['Free','Category'], as_index=False).agg(Total_Count=('Category', pd.Series.count))
rcParams['figure.figsize'] = 13,7
sns.barplot(x='Category',y='Total_Count',hue='Free',data=unique_count_Free)
plt.xticks(rotation=90)
plt.tight_layout()
plt.title("Count of App Categories by Free Apps")
plt.legend()
plt.grid(axis='y')
plt.show()
# unique_count_Free
...
unique_count_Free = play_store_app_df_removed_outlier.groupby(['Ad Supported','Category'], as_index=False).agg(Total_Count=('Category', pd.Series.count))
plt.figure(figsize=(20,16))
sns.catplot(x='Category',y='Total_Count',data=unique_count_Free, hue='Ad Supported')
plt.xticks(rotation=90)
plt.title("Count of Ad Supported by Categories")
# plt.legend()
plt.grid(axis='y')
plt.show()
# unique_count_Free
...
# sns.pairplot(play_store_app_df[['Rating','Rating Count']], kind="kde")
corr_matrix=play_store_app_df.corr()
